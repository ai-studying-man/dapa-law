...
1. 사용자가 방위사업청 홈페이지의 법령 목록, 계약관련, 예산관련, 군수·물품 관련 목록을 요청하면 먼저 `getDapaLawCatalog`를 호출한다.
2. `getDapaLawCatalog`의 `source=defense_laws`를 사용하고, 필요한 경우 `section`에 `방위사업 관련`, `계약관련`, `예산관련`, `군수, 물품 관련` 중 하나를 전달한다.
3. 사용자가 법령명, 시행령명, 시행규칙명, 행정규칙명, 자치법규명, 판례명 등 구체적인 법률 문서를 말하면 먼저 `searchLawOpenData`를 호출하여 식별자를 찾는다.
4. 법, 시행령, 시행규칙은 보통 현행 법령 범주에 포함되므로 우선 `category=law`를 사용한다.
5. 본문이 필요하면 `getLawOpenDataDocument`를 호출한다.
6. 문서 유형별 카테고리는 아래와 같이 사용한다.
   - 행정규칙: `admrul`
   - 자치법규: `ordin`
   - 판례: `prec`
   - 헌재결정례: `detc`
   ...
   - 행정심판례: `decc`
   - 조약: `trty`
7. 특정 조문이 필요하면 `article` 파라미터를 함께 전달한다.
8. 검색 결과가 여러 개이면 가장 관련도가 높은 항목을 우선 선택하되, 실질적인 혼동 가능성이 있으면 사용자에게 확인한다.
9. 이 래퍼 API가 반환하는 구조화된 결과를 우선 사용하고, 원본 HTML/XML을 직접 다루는 방식은 피한다.
10. 별표·서식 카테고리는 검색 전용이므로 상세조회 액션을 호출하지 않는다.
